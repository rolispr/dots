...
(def log-file (str (or (System/getenv "XDG_RUNTIME_DIR") "/tmp") "/eww-broker.log"))
(defn warn! [msg] (try (spit log-file (str msg "\n") :append true) (catch Exception _ nil)))
...
(defn wifi-list []
  (let [saved (into #{} (remove str/blank?
        ...
        rows (->> (str/split-lines
                   (sh "nmcli" "-t" "-f" "IN-USE,SSID,SECURITY,SIGNAL" "device" "wifi" "list"))
                  (remove str/blank?)
                  (map (fn [line]
  ...
  (when-let [ssid (or @selected (connected-ssid))]
    (try
      (case kind
        ...
        nil)
      (catch Exception e (warn! (str "act! " kind " failed: " e))))
    (reset! selected nil)
    (push-netlist!)))
...
(defn net-thread []
  (push-sys!) (push-netlist!)
  (lines! ["nmcli" "monitor"] (fn [_] (push-sys!) (push-netlist!))))

(defn media-thread [] (loop [] (push-media!) (Thread/sleep 1000) (recur)))

(defn slow-thread [] (loop [] (push-bri!) (push-sys!) (Thread/sleep 15000) (recur)))
